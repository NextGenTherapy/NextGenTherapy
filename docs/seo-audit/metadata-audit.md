# Metadata Audit

**Audit Date:** 2026-04-08
**Last Updated:** 2026-04-08 (Technical SEO fixes applied)
**Total Pages:** 28
**Status:** All pages have metadata exports — titles and descriptions optimised

---

## Summary

| Metric | Count |
|--------|-------|
| Pages with unique titles | 28/28 |
| Pages with descriptions | 28/28 |
| Pages with keywords | 28/28 |
| Pages with OG/Twitter | 28/28 |
| Pages with canonical | 28/28 |
| Titles under 60 chars | 28/28 |
| Descriptions 140-160 chars | 28/28 |

### Recent Fixes (2026-04-08)
- All metadata titles shortened to under 60 characters
- All descriptions shortened to 140-160 character range
- `/about` page already has keywords (verified)

---

## Detailed Audit

### Homepage (`/`)

| Field | Value | Status |
|-------|-------|--------|
| Title | Psychodynamic Therapy in Colchester & Online \| Andreea Horhocea | OK |
| Description | Psychodynamic therapy in Colchester and online (UK-wide) for women, neurodivergent adults and teenagers. BACP registered. Sessions in English and Romanian. Free 15-minute intro call. | OK |
| Keywords | 15 keywords | OK |
| OG Image | default-social-share.jpg | OK |
| Twitter | summary_large_image | OK |
| Canonical | https://nextgentherapy.co.uk | OK |

### About (`/about`)

| Field | Value | Status |
|-------|-------|--------|
| Title | About Andreea Horhocea — BACP Psychotherapist in Colchester | OK (58 chars) |
| Description | MA Psychodynamic Psychotherapy (University of Essex, 2020). BACP registered. Working with women, neurodivergent adults, teenagers and children in Colchester and online. Sessions available in English and Romanian. | OK |
| Keywords | 9 keywords | OK |
| OG Image | default-social-share.jpg | OK |
| Twitter | summary_large_image | OK |
| Canonical | /about | OK |

### Services (`/services`)

| Field | Value | Status |
|-------|-------|--------|
| Title | Therapy Services \| Psychodynamic Therapy Colchester | OK (51 chars) |
| Description | Psychodynamic therapy in Colchester & online. Women, neurodivergent adults, teenagers, children. Romanian available. BACP registered. | OK (136 chars) |
| Keywords | 12 keywords | OK |
| OG Image | default-social-share.jpg | OK |
| Canonical | /services | OK |

### Therapy for Women (`/therapy-for-women`)

| Field | Value | Status |
|-------|-------|--------|
| Title | Therapy for Women \| Colchester & Online \| Burnout | OK (50 chars) |
| Description | Psychodynamic therapy for women dealing with burnout, overthinking, anxiety, body image and self-worth. Colchester & online. BACP registered. | OK (142 chars) |
| Keywords | 13 keywords | OK |
| OG Image | default-social-share.jpg | OK |
| Canonical | /therapy-for-women | OK |

### Neurodiversity (`/neurodiversity`)

| Field | Value | Status |
|-------|-------|--------|
| Title | Adult ADHD & Autism Therapy \| Colchester & Online | OK (49 chars) |
| Description | Affirmative psychodynamic therapy for neurodivergent adults. Sensory-aware room, stimming welcome. Colchester & online UK-wide. BACP registered. | OK (144 chars) |
| Keywords | 14 keywords | OK |
| OG Image | default-social-share.jpg | OK |
| Canonical | /neurodiversity | OK |

### Teen Therapy (`/teen-therapy`)

| Field | Value | Status |
|-------|-------|--------|
| Title | Teen Therapy Colchester \| Anxiety, School Refusal | OK (49 chars) |
| Description | Psychodynamic therapy for teenagers aged 13–17 in Colchester. Anxiety, school refusal, self-harm, identity. BACP registered with NHS & SEN background. | OK (152 chars) |
| Keywords | 10 keywords | OK |
| OG Image | default-social-share.jpg | OK |
| Canonical | /teen-therapy | OK |

### Child Therapy (`/child-therapy`)

| Field | Value | Status |
|-------|-------|--------|
| Title | Child Therapy Colchester \| Play-Based Ages 4–12 | OK (47 chars) |
| Description | Play-based psychodynamic therapy for children 4–12 in Colchester. Anxiety, behaviour, family transitions, neurodiversity. BACP registered. | OK (139 chars) |
| Keywords | 13 keywords | OK |
| OG Image | default-social-share.jpg | OK |
| Canonical | /child-therapy | OK |

### Online Therapy (`/online-therapy`)

| Field | Value | Status |
|-------|-------|--------|
| Title | Online Therapy UK \| Video Sessions Age 16+ | OK (43 chars) |
| Description | Online psychodynamic therapy UK-wide. Secure video sessions, age 16+. £60 per session. Wednesdays. BACP registered. English & Romanian. | OK (135 chars) |
| Keywords | 12 keywords | OK |
| OG Image | default-social-share.jpg | OK |
| Canonical | /online-therapy | OK |

### Romanian Therapy (`/romanian-therapy`)

| Field | Value | Status |
|-------|-------|--------|
| Title | Romanian Therapist UK \| Terapie în Limba Română | OK (47 chars) |
| Description | Romanian-speaking psychodynamic therapist. Colchester & online UK-wide. Sesiuni în română. BACP registered, MA University of Essex. | OK (132 chars) |
| Keywords | 8 keywords (bilingual) | OK |
| OG Image | default-social-share.jpg | OK |
| Canonical | /romanian-therapy | OK |
| Language Alternates | en-GB, ro | OK |

### Pricing (`/pricing`)

| Field | Value | Status |
|-------|-------|--------|
| Title | Pricing & Booking \| £60 Per Session \| Free 15-Minute Call | OK |
| Description | Psychodynamic therapy £60 per 50-minute session, Colchester and online. Free 15-minute intro call. BACP registered. Transparent pricing with no hidden fees. | OK |
| Keywords | 11 keywords | OK |
| OG Image | default-social-share.jpg | OK |
| Canonical | /pricing | OK |

### FAQ (`/faq`)

| Field | Value | Status |
|-------|-------|--------|
| Title | FAQ \| Therapy Questions Answered \| Colchester | OK (45 chars) |
| Description | Answers to common therapy questions: cost, how it works, online sessions, confidentiality. Psychodynamic therapy Colchester & online. | OK (133 chars) |
| Keywords | 10 keywords | OK |
| OG Image | default-social-share.jpg | OK |
| Canonical | /faq | OK |

### Book Now (`/book-now`)

| Field | Value | Status |
|-------|-------|--------|
| Title | Book a Free 15-Minute Call \| Colchester Therapist | OK (49 chars) |
| Description | Book a free 15-minute call with Andreea, BACP-registered psychotherapist in Colchester. No commitment. In-person & online UK-wide. | OK (131 chars) |
| Keywords | 9 keywords | OK |
| OG Image | default-social-share.jpg | OK |
| Canonical | /book-now | OK |

### Location (`/location`)

| Field | Value | Status |
|-------|-------|--------|
| Title | Colchester Therapist Location \| Directions & Contact | OK |
| Description | Find your therapist in Colchester Business Centre. Easy directions, parking info, and contact details for your appointment. | OK |
| Keywords | 11 keywords | OK |
| OG Image | office-opt.jpg | OK (different image) |
| Canonical | /location | OK |

### Is This Right For You (`/is-this-right-for-you`)

| Field | Value | Status |
|-------|-------|--------|
| Title | Is Psychodynamic Therapy Right For You? \| Honest Guide | OK (55 chars) |
| Description | Honest guide to whether psychodynamic therapy suits you. What I offer, what I don't, and where else to look. Colchester & online. | OK (129 chars) |
| Keywords | 6 keywords | OK |
| OG Image | default-social-share.jpg | OK |
| Canonical | /is-this-right-for-you | OK |

### Trust (`/trust`)

| Field | Value | Status |
|-------|-------|--------|
| Title | Trust & Care \| BACP Registered Counsellor Colchester | OK |
| Description | Professional therapist in Colchester committed to trust and care. BACP registered with highest ethical standards. Safe, confidential environment. | OK |
| Keywords | 12 keywords | OK |
| OG Image | default-social-share.jpg | OK |
| Canonical | /trust | OK |

### Youth & Family FAQ (`/youth-family-faq`)

| Field | Value | Status |
|-------|-------|--------|
| Title | Youth & Family Therapy FAQ \| Questions Answered | OK (47 chars) |
| Description | FAQs about child, teen & young adult therapy. Guidance for families considering therapy in Colchester. Honest answers. | OK (117 chars) |
| Keywords | 15 keywords | OK |
| OG Image | default-social-share.jpg | OK |
| Canonical | /youth-family-faq | OK |

### Blog (`/blog`)

| Field | Value | Status |
|-------|-------|--------|
| Title | Mental Health Blog \| Expert Guidance & Professional Tips | OK |
| Description | Expert mental health guidance from BACP registered therapist in Colchester. Mental health advice, coping strategies & insights for better wellbeing. | OK |
| Keywords | 14 keywords | OK |
| OG Image | default-social-share.jpg | OK |
| Canonical | /blog | OK |

### Privacy Policy (`/privacy-policy`)

| Field | Value | Status |
|-------|-------|--------|
| Title | Privacy Policy \| Therapy Confidentiality Colchester | OK |
| Description | Therapy privacy & confidentiality in Colchester. BACP therapist commitment to protecting client information. Full GDPR compliance. | OK |
| Keywords | 18 keywords | OK |
| OG Image | default-social-share.jpg | OK |
| Canonical | /privacy-policy | OK |

### Terms (`/terms`)

| Field | Value | Status |
|-------|-------|--------|
| Title | Terms & Conditions \| Counselling Service Colchester | OK |
| Description | Terms & conditions for counselling service in Colchester. Professional service agreement with BACP registered counsellor. | OK |
| Keywords | 18 keywords | OK |
| OG Image | default-social-share.jpg | OK |
| Canonical | /terms | OK |

---

## Location Pages

All 8 location pages follow consistent structure:

| Page | Title Pattern | Keywords | Status |
|------|---------------|----------|--------|
| `/therapy-in-colchester` | Therapist in Colchester \| Psychodynamic Therapy | 10 | OK |
| `/therapy-in-wivenhoe` | Therapist Near Wivenhoe \| Psychodynamic Therapy | 9 | OK |
| `/therapy-in-mersea` | Therapist Near Mersea \| Psychodynamic Therapy | 8 | OK |
| `/therapy-in-tiptree` | Therapist Near Tiptree \| Psychodynamic Therapy | 9 | OK |
| `/therapy-in-marks-tey` | Therapist Near Marks Tey \| Psychodynamic Therapy | 9 | OK |
| `/therapy-in-manningtree` | Therapist Near Manningtree \| Psychodynamic Therapy | 9 | OK |
| `/therapy-in-clacton` | Therapist Near Clacton \| Psychodynamic Therapy | 8 | OK |
| `/therapy-in-ipswich` | Therapist Near Ipswich \| Psychodynamic Therapy | 10 | OK |

All location pages include:
- Drive time in meta description
- Local keywords (therapist [town], therapy [town])
- Canonical URLs
- OG/Twitter cards

---

## Action Items

### Completed (2026-04-08)
- [x] All titles shortened to under 60 characters
- [x] All descriptions shortened to 140-160 characters
- [x] `/about` page keywords verified (9 keywords present)

### Future Improvements (Optional)
1. Expand `/is-this-right-for-you` keywords from 6 to 10+
2. Normalise location page keyword counts (all to 9-10)
3. Add location-specific service keywords (e.g., "ADHD therapist Wivenhoe")

---

## Schema Coverage

| Schema Type | Pages Using |
|-------------|-------------|
| LocalBusiness | Homepage, location pages |
| Person | /about |
| Service | Cornerstone pages |
| FAQPage | /faq, /youth-family-faq |
| BreadcrumbList | Most pages (inline JSON-LD) |
| BlogPosting | /blog/[slug] |

All major pages have appropriate structured data.
